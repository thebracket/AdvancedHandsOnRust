...
fn movement(
  ...
  let Ok((entity, mut transform)) = player_query.single_mut() 
  else { return; };
    ...
    for y in 0 .. self.height {
      for x in 0 .. self.width {
        if self.solid[self.mapidx(x, y)] {
          ...
          if x==0 || x > self.width-3 || y==0 || y > self.height-3 {// <callout id="mbo.col.bounds" />
            // On the edge
            needs_physics = true;
          } else {
            // Are we surrounded by solid tiles?
            let solid_count = // <callout id="mbo.col.count" />
                self.solid[self.mapidx(x-1, y)] as u8
              + self.solid[self.mapidx(x+1, y)] as u8
              + self.solid[self.mapidx(x, y-1)] as u8
              + self.solid[self.mapidx(x, y+1)] as u8;
            if solid_count < 4 {// <callout id="mbo.col.check_count" />
              needs_physics = true;
            }
          }
          
          if  needs_physics {// <callout id="mbo.col.optional_push" />
            tile_positions.push((left + 12.0, top + 12.0));
          }
          //END: CollisionOptimize
        }
      }
    }
