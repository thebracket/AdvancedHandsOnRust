...
//START: setup
pub(crate) fn setup<T>(// <callout id="flappy_menu.menu_setup" />
  state: Res<State<T>>,
  ...
  T: States+FromWorld+FreelyMutableState,
{
  ...
  let current_state = state.get();// <callout id="flappy_menu.fetch_state" />
  ...
  commands
    .spawn(Camera2d::default())// <callout id="flappy_menu.spawn_camera" />
    .insert(MenuElement);
  commands
    .spawn(( Sprite { 
        image: menu_graphic, 
