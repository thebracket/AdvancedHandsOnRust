use bevy::prelude::*;
use my_library_flappy_collision::*;
...
fn main() -> anyhow::Result<()> {
  ...
  add_phase!(app, GamePhase, GamePhase::Flapping,
    ...
    run => [ flap, clamp, move_walls, hit_wall, cycle_animations,
      ...
      //START_HIGHLIGHT
      apply_velocity, check_collisions::<Flappy, Obstacle> ],
      //END_HIGHLIGHT
    //END: apply_systems
...
//START: BuildWall
fn build_wall(commands: &mut Commands, assets: &AssetStore, gap_y: i32, 
...
fn flap(
  ...
  //START_HIGHLIGHT
  mut impulse: EventWriter<Impulse>, //END_HIGHLIGHT
) {
